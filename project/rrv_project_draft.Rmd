...
```bach
blastn -query query.fasta -db /scratch/biol726310/BIOL7263_Genomics/rrv_project/databases/virus_db -out results.csv -evalue 1e-5 -max_target_seqs 5 -num_threads 4 -outfmt "10 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore"
```
...
The following code is supposed to generate a `.csv` file type that will be much easier to analyze and can be quickly summarized.

```bach
diamond blastx -d /scratch/biol726310/BIOL7263_Genomics/rrv_project/databases/protein_db -q query.fasta --outfmt 6 qseqid sseqid stitle pident length mismatch gapopen qstart qend sstart send evalue bitscore --max-target-seqs 5 --threads 8 | sed 's/\t/,/g' > results.csv
```
...
### **9. Accumulate and analyze BLAST results for both nucleotide and protein blast**
...
> Now we can look at the data obtained from the search. We can look at a couple of parameters such as the coverage, percent ID, 

```{r, eval=FALSE}
df_complete %>% arrange(desc(LENGTH))

df_complete %>% arrange(desc(COV))

df_complete %>% arrange(desc(Percent_ID))

df_complete %>% arrange(desc(Align_Length))

df_complete %>% arrange(Mismatch)
```
...
reduced_data %>% arrange(desc(COV))

reduced_data %>% arrange(desc(Percent_ID))

reduced_data %>% filter(COV >= 10) %>% 
...
> Ideally, it would be nice to generate a master document with most of the programs so that we can input a raw HTS sequence, and end with blast results. Realistically, I think we can break up the data processing into two main steps 1) Fastqc, trimming, and reference mapping 2) Assembly, Quast, Diamond, and blast_n
